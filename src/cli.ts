...
const program0 = new Command()
  ...
  .argument('[files...]', 'One JSX file or deck directory (omit or use - for stdin)', ['-'])
...
const program = output_options(program0)
  ...
  .action(async (files: string[], values: CliOptions) => {
    ...
    if (inputs.multi) {
      const { deck } = inputs
      const options = { width: values.width, height: values.height }
      ...
      const options = { theme, width: values.width, height: values.height }
